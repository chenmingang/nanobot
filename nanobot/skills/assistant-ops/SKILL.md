---
name: assistant-ops
description: 对当前 nanobot 助手本身的操作。包括使用 gateway.sh 重启/启停、使用 memory_cli 脚本操作记忆、使用 nanobot 命令处理 session。用户说「重启助手」「清空会话」「整理记忆」等时使用本 skill。
---

# Assistant Ops Skill（助手自身操作）

本 skill 用于操作**当前 nanobot 助手本身**：重启网关、操作记忆、管理会话。记忆部分使用本 skill 下的 **memory_cli** 脚本（exec）；网关与 session 通过 **exec** 执行命令，执行前请确认工作目录或 PATH。
...
## 2. 记忆操作（memory_cli 脚本）

- 记忆相关工具已不对模型暴露，需通过本 skill 下的 **memory_cli** 脚本操作（exec）。
- **脚本**：exec 时 working_dir 设为 **代码目录/nanobot/skills/assistant-ops**，命令用 `python scripts/memory_cli.py <子命令> [参数]`（模型能读到本 skill 目录，故可直接用 scripts/...）。
- 记忆目录为配置的 workspace/memory，脚本自动读配置。
- **示例**：`python scripts/memory_cli.py remember "内容"`、`append_daily "内容"`、`organize_memory`、`get memory/MEMORY.md`、`list`。
...
- **记忆**：用 exec 运行本 skill 下的 `nanobot/skills/assistant-ops/scripts/memory_cli.py`（模型不暴露记忆工具）。
- **会话**：list 查看；clear 慎用（清空所有）；delete 按 key 删除单会话。执行 clear/delete 前可根据需要先 list 确认。
- 所有命令均通过 **exec** 执行；**working_dir 必须为代码目录**（与 gateway.sh 同级），这样 `gateway.sh` 与 `nanobot/skills/assistant-ops/scripts/memory_cli.py` 才能被正确找到。
